...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($username) || empty($password)) {
        ...
        if ($stmt->num_rows > 0) {
            ...
            } elseif ($profile_photo["size"] > 5000000) {
                $error = "File is too large. Maximum 5MB.";
            } elseif (move_uploaded_file($profile_photo["tmp_name"], $target_file)) {
                // Hash password
                $hashed_password = password_hash($password, PASSWORD_DEFAULT);

                // Insert into database
                $stmt = $conn->prepare("INSERT INTO users (username, password, profile_photo) VALUES (?, ?, ?)");
...
<head>
    ...
    <title>Register - Gaming Friendship Hub</title>
    ...
    <style>
        ...
        nav {
            ...
            position: fixed;
            top: 90px; /* Adjusted to stick below header */
            width: 100%;
            z-index: 999;
        }
        nav a {
        ...
        .container {
            max-width: 1000px;
            margin: 150px auto 30px; /* Adjusted margin-top to account for fixed nav */
            padding: 25px;
